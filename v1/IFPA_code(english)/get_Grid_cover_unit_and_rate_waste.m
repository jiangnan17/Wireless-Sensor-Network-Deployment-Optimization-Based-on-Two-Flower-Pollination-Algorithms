%%Solve the network coverage rate according to formula 1-4 in the paper, and solve the node radiation overflow rate through formula 13-15
function [cover_rate,waste_rate] = get_Grid_cover_unit_and_rate_waste(sensor_mat1,sersors_r,per_sersons_num,per_sersons_radius_type)
    %Proportion of waste = (ideal coverage-true coverage-overlapping coverage) / ideal coverage
    global M;
    global N;
 ...
 % The stupidest way to write them separately
 % Perform data processing by intercepting
    fiftynum = (1:1:50);
    ...
    Grid_cover_unit = zeros(L,W);% 1 * M joint probabilities for saving 2500 grids
    Grid_cover_bool = zeros(L,W,N);%Probability of monitoring at each grid center
   
    %Here we also have to consider involving the overlapping area
    for i=1:L
		for k=1:W
            ...
            % Judgment of diamond-shaped obstacles
			...
			for j=1:N
				if ((Grid_cen_x_and_y(i,k,1)-sensor_mat1(1,j))^2 + (Grid_cen_x_and_y(i,k,2)-sensor_mat1(2,j))^2)...
						<=sersors_r(1,j)^2
					Grid_cover_bool(i,k,j) = 1;%Represents that the center point of the i-th grid can be covered by the j-th sensor node
				end
			end
		end
    end
    ...
    %% calculate the joint distribution probability
    for i=1:L
		for k=1:W
          ...
          % Judgment of diamond-shaped obstacles
			...
			p = 1;%Set to 1
			for j=1:N
				p = p*(1-Grid_cover_bool(i,k,j));
			end
			Grid_cover_unit(i,k) = 1-p;% Joint coverage probability
		end
    end

   % Calculated coverage
    p_sum = 0;% Cumulative coverage probability of all grids
    for i=1:L
        for j=1:W
    ...
    cover_rate = p_sum*(L*W/M)/((L*W)- 480 - 210);% Coverage% Note that here, in fact, the compromise here is 480, not 450 is calculated according to the point is 512, the diamond point is 220, the actual area is 200, the compromise is 210
    ...
    % Statistical overlapping area
    area_over = 0;% Overlapping area
    Grid_cover_over_matri = zeros(L,W);
    for i=1:L
        for j=1:W
            Grid_cover_over_matri(i,j) = sum(Grid_cover_bool(i,j,:));% Count how many times a monitoring point is covered
        end
    end
   % Calculate the number of times each monitoring point is covered repeatedly
    for i=1:L
        for j=1:W
            if Grid_cover_over_matri(i,j) >= 2
                area_over  = area_over + Grid_cover_over_matri(i,j) - 1;% Repeat n-1 times
            end
        end
    end
   ...
   % Use grid points to calculate, otherwise there may be negative numbers because grid points have some errors
    r_max_area = 158;
    r_mid_area = 117;
    r_min_area = 81;
    % Storage ideal coverage area
