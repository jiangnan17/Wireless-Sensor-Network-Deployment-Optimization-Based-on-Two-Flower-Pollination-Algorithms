...
for i=1:sizepop
    ...
    for j=1:N
        ...
        if (struct_pops(i).per(1,j)>=15&&struct_pops(i).per(1,j)<=35) && (struct_pops(i).per(2,j)>=35&&struct_pops(i).per(2,j)<=40)%说明在障碍物里面了
        %也是要均匀的放置 均匀放在障碍物的周围 
            if rem(j,10)==0||rem(j,10)==1||rem(j,10)==2||rem(j,10)==3 %上
                struct_pops(i).per(2,j) = 40+rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==4||rem(j,10)==5||rem(j,10)==6||rem(j,10)==7 %下
                struct_pops(i).per(2,j) = 35-rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==8  %左
                struct_pops(i).per(1,j) = 15-rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==9%右
                struct_pops(i).per(1,j) = 35+rand(1,1)*struct_pops(i).radius(1,j);
            end
        end
        
         %处理矩形下
         %障碍物处理  和周围处理  且根据比例来进行分配  上 下各为4  左右各为1 共10
        if (struct_pops(i).per(1,j)>=15&&struct_pops(i).per(1,j)<=35) && (struct_pops(i).per(2,j)>=10&&struct_pops(i).per(2,j)<=15)%说明在障碍物里面了
        %也是要均匀的放置 均匀放在障碍物的周围 
            if rem(j,10)==0||rem(j,10)==1||rem(j,10)==2||rem(j,10)==3 %上
                struct_pops(i).per(2,j) = 15+rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==4||rem(j,10)==5||rem(j,10)==6||rem(j,10)==7 %下
                struct_pops(i).per(2,j) = 10-rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==8  %左
                struct_pops(i).per(1,j) = 15-rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==9%右
                struct_pops(i).per(1,j) = 35+rand(1,1)*struct_pops(i).radius(1,j);
        ...
        if (struct_pops(i).per(1,j)>=22&&struct_pops(i).per(1,j)<=28) && (struct_pops(i).per(2,j)>=15&&struct_pops(i).per(2,j)<=35)%说明在障碍物里面了
        %也是要均匀的放置 均匀放在障碍物的周围 
            if rem(j,10)==0||rem(j,10)==1||rem(j,10)==2||rem(j,10)==3||rem(j,10)==4 %左
                struct_pops(i).per(1,j) = 22-rand(1,1)*struct_pops(i).radius(1,j);
            elseif rem(j,10)==5||rem(j,10)==6||rem(j,10)==7||rem(j,10)==8||rem(j,10)==9 %右
                struct_pops(i).per(1,j) = 28+rand(1,1)*struct_pops(i).radius(1,j);
            end
        end
